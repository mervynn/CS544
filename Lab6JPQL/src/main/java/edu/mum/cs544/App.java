...
public class App {
    ...
    public static void main(String[] args) throws Exception {
        ...
        List<Flight> flights = em.createQuery("from Flight f " +
                "where f.airplane.capacity > 500 " +
                "and f.origin.country = 'USA' " +
                "and f.destination.country <> 'USA'", Flight.class).getResultList();
        ...
        flights = em.createQuery("from Flight f where f.airplane.model = '747' " +
                "and f.airline.name <> 'Star Alliance'", Flight.class).getResultList();
        System.out.printf("%-9s%-31s%-31s\n", "Flight:", "Departs:",
                "Arrives:");
        for (Flight flight : flights) {
            System.out.printf("%-7s  %-12s %7s %8s  %-12s %7s %8s\n",
